...
#if (false)
#ifdef __GNUC__
uint8_t g_display0_fb_foreground[2][((800 * 480) * DISPLAY_BITS_PER_PIXEL_INPUT1) >> 3] __attribute__ ((aligned(64), section(".sdram")));
#else /* __ICCARM__ */
#pragma data_alignment = 64
#pragma location = ".sdram"
uint8_t g_display0_fb_foreground[2][((800 * 480) * DISPLAY_BITS_PER_PIXEL_INPUT1) >> 3];
#endif
#else
...
#if false
#ifdef __GNUC__
uint8_t g_sf_el_gx_canvas[sizeof(g_display0_fb_background[0])] __attribute__ ((aligned(4), section(".sdram")));
#else /* __ICCARM__ */
...
sf_message_ctrl_t g_sf_message0_ctrl;
...
{
    ssp_err_t err_g_sf_message0;

    /* Initializes Messaging Framework Queues */
    g_message_init ();

    /* Opens the messaging framework */
    err_g_sf_message0 = g_sf_message0.p_api->open (g_sf_message0.p_ctrl, g_sf_message0.p_cfg);
    if (SSP_SUCCESS != err_g_sf_message0)
    {
